...
/*function uv_ns_httpget ($url,$timeout,$depth)
  ...
  {  
   ...
   #update mdl_uv_config set valor = '0' where nombre = 'bancuv';
...
function get_community_key ($cid) {
  #DRG todos las comunidades/ o cursos migrados deberian tener idnumber para poder referenciarlos al migrar de plataforma
  global $DB, $CFG;
...
function uv_get_server_ws_academico () {
    #Para ajustar el server de bancuv a PRO o DESA bancuv2
    global $DB;
    $source = "bancuv.uv.es";
    return $source;
}
...
function getUrlEncuesta($npa,$asignatura,$grupo) {
   #DRG comprueba si tiene encuestas activas
   ...
   #PENDIENTE          
   #4.
   #Sanchis Llopis, Maria Teresa
   #ENVIADA

   $server=  uv_get_server_ws_academico();
   $url   =  "http://".$server."/pls/uv0/paramoodle.enc_evaluacion_alumno?wnpa=$npa&wasignatura=$asignatura&wgrupo=$grupo&wdetalle=2";
   #m("DRG urlEncuesta $url");
   $urlData = uv_get_curl($url);
   
   #$urlreturn = "NoDataFound";
   #m("DRG urlData $urlData");
   #Fatal error si no esta permitido el servidor al llamar a bancuv
   if ( $urlData == "0" ) {
      $urlreturn = "Desactivado";
      m("DRG if urlData 0");
   } elseif ($urlData == "No data found") {
      $urlreturn = "NoDataFound";
      #m("DRG if NoDataFound");
   } elseif (preg_match ("Fatal error",$urlData)) {
      $urlreturn = "No permitido";
   } else {
      ...
      if (preg_match("/PENDIENTE/",$urlData)) {
          ...
          $urlreturn = "NoDataFound";
          #m("DRG else NoDataFound");
      }
   }
   #m("DRG urlreturn $urlreturn");
...
function getNumparFromCacadFromBancuv($cacad) {
   ...
   //bancuv.uv.es/pls/uv0/paradotcen.numpar_any?wany=2016
   $server = uv_get_server_ws_academico();
   $url =  "http://".$server."/pls/uv0/paradotcen.numpar_any?wany=$cacad";
...
function process_file_photo ($file, $userfield, $overwrite) {
    ...
    #DRG añadimos false para que no convierta las fotos de jpg a png
    $newrev =  process_new_icon($context, 'user', 'icon', 0, $file, false);
...
//DRG coge numpar actual
function get_numpar_uv () {
  global $DB;
  $numpar=0;
  $numpar = $DB->get_field('uv_config','valor', array('nombre'=>'numpar'));
  return $numpar;
}

function get_var ($token, $xdefault = '', $xtype = 'REQUEST') {
  if ($xtype == "REQUEST") {
    #m("DRG var $token");
    #m("DRG value $_REQUEST[$token]");
    ...
    $res = $_REQUEST[$token];
    #m (json_encode($res));
    if ($res == "") return $xdefault;
    if (SQLInjectionTest ($res)) {
      echo "SLQInjection detected!";
      exit;
    }
    return $res;
  } else if ($xtype == "SESSION") {
    if (isset ($_SESSION[$token]) == false) return $xdefault;
    $res = $_SESSION[$token];
    if ($res == "") return $xdefault;
    if (SQLInjectionTest ($res)) {
      echo "SLQInjection detected!";
...
function SQLInjectionTest($checkstring) {
 // syslog(LOG_NOTICE, "SCT SQLInjectionTest $checkstring");
 //AGUSTIN 20171108
 //COMENTADO POR LOS STRINGS EN VALENCIANO
 // $p = strpos ($checkstring, "'"); if ($p !== false) {return TRUE;}
 #DRG evitar string script
 #m("DRG checkstring $checkstring ");
 $checkstring = pg_escape_string($checkstring);
 $sqltest = array (
    "/SELECT.*/i",
    "/INSERT.*/i",
    "/DELETE.*/i",
   ...
   #$validUTF8 = ! (false === mb_detect_encoding($token, 'UTF-8', true));
   #detecta % en el string indica caracter UTF-8
   if (!preg_match('//u', $checkstring)) {
    return TRUE;
   }
   return FALSE;
}

function check_center_uv ($numpar,$xcentro) {
 global $DB;
 $sql="select idnumber 
       from mdl_course_categories 
       where parent in (select id from mdl_course_categories where idnumber = :numpar)";
 $params['numpar']      = $numpar;
 $idnumberObject = $DB->get_records_sql($sql,$params);
 #$idnumberArray=var_dump( (array) $idnumberObject );
 $idnumberArray = (array) $idnumberObject;
 foreach ($idnumberArray as $id) {
  #m("DRG $id->idnumber")  ;
  if ( preg_match ($id->idnumber,$xcentro)) {
   if ('c000' != $xcentro) {   #No existe pero es una entrada valida
     return FALSE;
     #echo "El centro no existe en este numpar o no tiene cursos!";
     #exit;
   }
  }
  return TRUE;
...
function get_centers_uv ($numpar) {
 global $DB;
 $sql="select idnumber,name 
       from mdl_course_categories 
       where visible='1' and 
             idnumber like 'c%' and 
             parent in (select id from mdl_course_categories where idnumber = :numpar) 
       order by 1";
 $params['numpar']      = $numpar;
 $idnumberObject = $DB->get_records_sql($sql,$params);
 #$idnumberArray=var_dump( (array) $idnumberObject );
...
//set_tipo_customfield_data esta en traspasoslib
function get_fullname_customfield_uv ($cursoid,$idioma) {
       global $DB, $CFG, $SESSION, $USER;
       #DRG ver current_language function para mejorar el lenguaje
       #en config.php se activa/desactiva
       if (empty($idioma)) {
           setup_lang_from_browser();
           if (!empty($SESSION->lang)) {
             $idioma=$SESSION->lang;
           } else {
             #DRG al cambiar el idioma desde preferencias, hasta que no se cierra el navegador y vuelve a entrar no lo coge,
             #pq SESSION tiene valor. Al cambiar preferencias deberia cambiar SESSION
             $idioma=$USER->lang;
           }
       }
       #m("DRG C $cursoid I $idioma");
       $name_es='cfullname_es';
       $name_en='cfullname_en';
       $sql= "select f.shortname,d.value
              from mdl_customfield_field f, mdl_customfield_data d
              where d.fieldid=f.id and (shortname = ? or shortname =?) and instanceid = ?";
       $cns  = $DB->get_records_sql($sql, array($name_es,$name_en,$cursoid));
       foreach ($cns as $cn) {
         if ($cn->shortname=='cfullname_es') {
              $fullname_es= $cn->value;
         } elseif ($cn->shortname=='cfullname_en') {
              $fullname_en= $cn->value;
         }
       }
      if (!$course = $DB->get_record("course", array("id"=>$cursoid))) {
        $fullname_ca="ERROR AL OBTENER NOMBRE CURSO";
      } else {

        $fullname_ca=$course->fullname;
      }
       switch($idioma) {
            case 'ca':
                #DRG siempre existe
                $fullname= $fullname_ca;
                break;

            case 'es':
                #DRG si no existe poner ca
                if (empty($fullname_es)) {
                  $fullname= $fullname_ca;
                } else {
                  $fullname= $fullname_es;
                }
                break;
            case 'en':
                #DRG si no existe poner ca
                if (empty($fullname_en)) {
                  $fullname= $fullname_ca;
                } else {
...
function poner_acentos ($s)  {
 #DRG para que el filtro salgan los nombres con acentos en la primera vocal, si intenta poner acento a una consonante devuelve vacio
 #$vocales = array("Aa" => À, "Ac" => Á, "Ea" => È, "Ec" = > É, "Ia"=> Ì, "Ic"=> Í, "Oa" => Ò , "Oc"=>Ó, "Ua" =>Ù, "Uc"=>Ú);
 #Inicializó el array, solamente puede tener una letra por iteración
 $vocales = array();
 $vocales["Aa"] = str_replace ("A","À",$s);
 $vocales["Ac"] = str_replace ("A","Á",$s);

 $vocales["Ea"] = str_replace ("E","È",$s);
 $vocales["Ec"] = str_replace ("E","É",$s);

 $vocales["Ia"] = str_replace ("I","Ì",$s);
 $vocales["Ic"] = str_replace ("I","Í",$s);

 $vocales["Oa"] = str_replace ("O","Ò",$s);
 $vocales["Oc"] = str_replace ("O","Ó",$s);

 $vocales["Ua"] = str_replace ("U","Ù",$s);
 $vocales["Uc"] = str_replace ("U","Ú",$s);
 return $vocales;
}

function check_enrollment($useridnumber, $course, $role){
    #DRG: comprueba si pertenece a un curso, opcional rol
    global $DB;
    if ($useridnumber == "") {
     return False;
    }
    if ($role == 'student') {
     $role_where = "AND ra.roleid='5'";
    } elseif ($role == 'teacher') {
     $role_where = "AND ra.roleid='3'";
    } else {
     $role_where = "AND (ra.roleid='3' OR ra.roleid='5')";
    }
    ...
    $sql="SELECT count(*)
     ...
     INNER JOIN mdl_role r ON ra.roleid = r.id
     INNER JOIN mdl_user usr ON ra.userid = usr.id
     WHERE c.id = '$course' AND usr.idnumber='$useridnumber'
     $role_where";
    #m("DRG sql $sql");
    ...
    if($n==0) {
        ...
        add_to_log($course, 'ERROR: check-enrollment', 'Comprobacion encuesta');
        return False;
    }
}
